...
type MemoryVerseRepo interface {
	GenerateDailyVerse(ctx context.Context) error
	GetDailyVerse(ctx context.Context) (*Verse, error)
...
// Generate a random verse based on the specified translation

func (r *repository) GenerateDailyVerse(ctx context.Context) error {
	// Pick a random verse
	var verseID int
	err := r.db.QueryRowContext(ctx, `
        SELECT id FROM memory_verses
        ORDER BY RANDOM()
        LIMIT 1
    `).Scan(&verseID)
	...
	// Insert into daily_verse_log
	_, err = r.db.ExecContext(ctx, `
        INSERT INTO daily_verse_log (verse_id, verse_date)
...
// GetDailyVerse retrieves the verse assigned for today

// func (r *repository) GetDailyVerse(ctx context.Context) (*Verse, error) {
//     query := `
//         SELECT
//             mv.id, mv.reference, mv.verse, mv.translation, mv.created_at
//         FROM daily_verse_log dvl
//         JOIN memory_verses mv ON mv.id = dvl.verse_id
//         WHERE dvl.verse_date = CURRENT_DATE
//         LIMIT 1
//     `

//     var v Verse
//     err := r.db.QueryRowContext(ctx, query).
//         Scan(&v.ID, &v.Reference, &v.Verse, &v.Translation, &v.CreatedAt)
...
func (r *repository) GetDailyVerse(ctx context.Context) (*Verse, error) {
	query := `
        SELECT 
            id, reference, verse, translation, created_at
        FROM memory_verses
        ORDER BY RANDOM()
        LIMIT 1
    `
